...
const authManagerUri = process.env.AUTH_MANAGER_SERVICE_URI!;
  ...
  try {
    ...
    if (token.user?.id) {
      try {
        ...
        // );
        // Test with the new service
        const response = await fetch(`${authManagerUri}/refresh-token`, {
          method: "post",
          headers: {
        ...
        console.info("[AUTH.ts]:[REFRESH_TOKEN]:RESPONSE", result);
      } catch (err) {
    ...
    console.info("[AUTH.ts]:[NEXT]:RESPONSE", token);
    return token;
  } catch (err) {
  ...
  callbacks: {
    async jwt({ token, account, user, profile, trigger, session }) {
      if (account && user) {
        ...
        console.info("[AUTH.ts]:[FIRST_LOGIN]:[NEXT]:RESPONSE", account);
        if (account.refresh_token && profile?.sub) {
          try {
            ...
            const result = await response.json();
            console.info(
              "[AUTH.ts]:[FIRST_LOGIN]:[REFRESH_TOKEN]:RESPONSE",
              result
            );
          } catch (err) {
            console.error("vault.error", {
